...
public class ExternalProductService {
    ...
    public ExternalProductService(@Value("${external.products.api.url:http://localhost:8081}") String baseUrl) {
    ...
    public List<ExternalProductDTO> getAllExternalProducts() {
        try {
            ...
            throw new ExternalServiceException("Failed to fetch products from external API: " + e.getMessage());
        }
    }

    public Optional<ExternalProductDTO> getExternalProductById(Long id) {
        try {
            ExternalProductDTO product = restClient.get()
                    .uri("/products/{id}", id)
                    .retrieve()
                    .body(ExternalProductDTO.class);
            return Optional.ofNullable(product);
        } catch (RestClientException e) {
            throw new ExternalServiceException("Failed to fetch product with id " + id + ": " + e.getMessage());
        }
    }
    ...
    public ExternalProductDTO createExternalProduct(ExternalProductDTO product) {
        try {
            ...
            throw new ExternalServiceException("Failed to create product in external API: " + e.getMessage());
        }
    }
}